...
// testBlockChain is a simple in-memory blockchain for testing
...
func (bc *testBlockChain) InitModule(*modules.Core) {}

func (bc *testBlockChain) Store(block *hotstuff.Block) {
	bc.blocks[block.Hash()] = block
}

func (bc *testBlockChain) LocalGet(hash hotstuff.Hash) (*hotstuff.Block, bool) {
	block, ok := bc.blocks[hash]
	return block, ok
}

func (bc *testBlockChain) Get(hash hotstuff.Hash) (*hotstuff.Block, bool) {
	return bc.LocalGet(hash)
}

func (bc *testBlockChain) Extends(block, ancestor *hotstuff.Block) bool {
	return true // simplified for testing
}

func (bc *testBlockChain) PruneToHeight(height hotstuff.View) {}
